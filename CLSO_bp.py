...
'''计算适应度函数'''
def cal_fitness(x, fun=BPNN, order=0, fitness=None):
...
def CaculateFitness_parallel(X, fun):
    ...
    res = pool.map(cal_fitness,X) 
    pool.close() #关闭进程池，关闭后po不再接收新的请求
    pool.join()
    
    fitness = np.array(res, dtype=np.float)
...
if __name__ == '__main__':
    ...
    MSE_loss = BPNN(pathX, w, b, epochs=10000, learning_rate=0.005, need_paint=True,need_loss_log=True)
    print(MSE_loss)
    logger.info(MSE_loss)
