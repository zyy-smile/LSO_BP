...
def initial(pop, dim, ub, lb):
    ...
    for i in range(dim - 1):
        for j in range(pop - 1):
            if X[j, i] == 0 or X[j, i] == 0.25 or X[j, i] == 0.5 or X[j, i] == 0.75:
                X[j, i] = X[j, i] + 0.1 * random.random()
            elif 0 <= X[j, i] < 0.5:
                X[j + 1, i] = 2 * X[j, i]
            elif 0.5 < X[j, i] <= 1:
                X[j + 1, i] = 2 * (1 - X[j, i])
...
def train_with_init_para(parameter_str):
    ...
    MSE_loss = BPNN(data, w, b, epochs=10000, need_paint=True)
    print(MSE_loss)
...
#     MSE_loss = BPNN(data, w, b, epochs=Maxepoch, learning_rate=0.005, need_paint=True,need_loss_log=True)
#     print('MSE_loss=',MSE_loss)
#     logger.info('MSE_loss={}'.format(MSE_loss))
#
#     # BP
#     test_set_input, test_set_output, predict, l_arr = BP(data,max_epoch=Maxepoch)
#     # 图1
#     plt.subplot(1, 2, 1)
